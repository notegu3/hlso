...
((s,r,h)=>{
    ...
    d = (()=>{
        ...
        return _createClass(e, null, [{
            ...
            value() {
                ...
                // The MAX cell count is enforced by the 3rb.io SERVER, not the client — the
                // client only sends split commands and the server stops splitting once you hit
                // its cap (ignoring any extra). Each split() doubles the pieces (2^steps), so 8
                // steps would reach 256 — high enough to ALWAYS hit 3rb.io's real maximum,
                // whatever it is. The few extra commands beyond the cap are harmless.
                // Tunable via window._3rbSplitSteps / _3rbSplitDelay.
                const steps = window._3rbSplitSteps || 6
					, delay = window._3rbSplitDelay || 40;
                this.split(n);
                for (let s = 1; s < steps; s++) {
                    setTimeout(()=>{ e.split(n); }, s * delay)
                }
            }
        }, {
            key: "commander",
    ...
    f = new ((()=>{
        ...
        return _createClass(e, [{
            ...
            value() {
                return {
                    regions: {
                        "US-Atlanta": { numPlayers: 0 },
                        "BR-Brazil": { numPlayers: 0 },
                        "EU-London": { numPlayers: 0 },
                        "RU-Russia": { numPlayers: 0 },
                        "TK-Turkey": { numPlayers: 0 },
                        "JP-Tokyo": { numPlayers: 0 },
                        "CN-China": { numPlayers: 0 },
                        "SG-Singapore": { numPlayers: 0 }
                    }
                };
            }
        }, {
    ...
    g = new ((()=>{
        ...
        return _createClass(e, [{
            ...
            value() {
                const e = r("#regions").val()
                  , t = r("#gamemode").val()
                  , o = f.getServer(e, t) || { endpoints: { https: "alpha2.3rb.io:1777/V5" } };
                if (o.token)
    ...
    m = (()=>{
        ...
        return _createClass(e, null, [{
            ...
            value() {
                ...
                this.emojiPath = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNkYAAAAAYAAjCB0C8AAAAASUVORK5CYII=#",
                this.emojis = {
                    ":01:": "angry.png",
                    ":02:": "angry-1.png",
                    ":03:": "cool.png",
            ...
            value() {
                const e = r("#emojiContainer")
                  , t = t=>{
                    const o = r('<img src="' + this.emojiPath + this.emojis[t] + '" class="emojiPreview">');
                    o.click(()=>{
                        const e = r("#message")
                          , o = e.val();
                        e.val(o + " " + t),
    ...
    const U = (()=>{
        ...
        return _createClass(e, null, [{
            ...
            value() {
                ...
                this.cmdSound = new Audio("data:audio/wav;base64,UklGRigAAABXQVZFZm10IBAAAAABAAEARKwAAIhYAQACABAAZGF0YQQAAAAAAAAD") || {
                    play: ()=>{}
                },
      ...
      , $ = new ((()=>{
        ...
        return _createClass(e, [{
            ...
            value() {
                ...
                s.FB ? FB.init(this.options) : null
            }
        }, {
            key: "init",
            value() {
            ...
            value() {
                ...
                for (const o of this.list.values()) {
					...
					try {
						...
						t.position.innerHTML = e;
						t.position.style.display = "";
                    } catch(err) {
						t.nick.innerHTML = this.cleanNick(o.nick);
					};
    ...
    S = new ((()=>{
        ...
        return _createClass(e, [{
            ...
            value() {
                // Removed to clean up the minimap (as per user request)
            }
        }, {
            key: "renderSelf",
            value() {
                ...
                window._hsloMinimapSkins = window._hsloMinimapSkins || {};
                const getSkinImg = (code) => {
                    if (!code) return null;
                    let url = N.code2url(code);
                    if (!url) return null;
                    let img = window._hsloMinimapSkins[url];
                    if (img) return (img.complete && img.naturalWidth > 0) ? img : null;
                    img = new Image();
                    img.crossOrigin = "anonymous";
                    img.src = url;
                    window._hsloMinimapSkins[url] = img;
                    return null;
                };

                const drawNode = (x, y, name, skinCode, isSelf) => {
                    // Draw glowing pulse
                    this.ctx.strokeStyle = "rgba(79, 236, 255, 0.4)";
                    this.ctx.lineWidth = 1.5;
                    this.ctx.beginPath();
                    this.ctx.arc(x, y, 8 + Math.sin(Date.now() / 150) * 2, 0, 2 * Math.PI);
                    this.ctx.stroke();

                    const r = 7;
                    let skinImg = getSkinImg(skinCode);
                    if (skinImg) {
                        this.ctx.save();
                        this.ctx.beginPath();
                        ...
                        this.ctx.closePath();
                        this.ctx.clip();
                        this.ctx.drawImage(skinImg, x - r, y - r, r * 2, r * 2);
                        this.ctx.restore();
                        this.ctx.lineWidth = 1.5;
                        this.ctx.strokeStyle = "#4fecff";
                        this.ctx.beginPath();
                        this.ctx.arc(x, y, r, 0, 2 * Math.PI);
                        this.ctx.stroke();
                    } else {
                        this.ctx.fillStyle = "#4fecff";
                        this.ctx.beginPath();
                    ...
                    if (name) {
                        this.ctx.font = "bold 13px Rajdhani, ubuntu, sans-serif";
                        this.ctx.textAlign = "center";
                        this.ctx.textBaseline = "bottom";
                        this.ctx.lineJoin = "round";
                        this.ctx.lineWidth = 2;
                        this.ctx.strokeStyle = "#000000";
                        this.ctx.strokeText(name, x, y - 10);
                        this.ctx.fillStyle = "#4fecff";
                ...
                if (A.isAliveTab1) {
                    drawNode(e1, t1, A.tab1Info ? A.tab1Info.nick : "Me 1", A.skin, true);
                    if (A.isAliveTab2) {
                        drawNode(e2, t2, A.tab2Info ? A.tab2Info.nick : "Me 2", A.skin2, true);
                    }
                } else if (A.isAlive) {
                    drawNode(e, t, "Me", A.skin, true);
                }
            }
        }, {
            key: "renderGhost",
            value() {
                // Removed to clean up the minimap
            }
        }, {
            key: "renderTeammates",
            value() {
                ...
                const drawTeammate = (i, xProp, yMapProp, nickProp, colorProp, skinProp, prefix = "") => {
                    i.animate();
                    var t = i[yMapProp === 'mapY' ? 'mapX' : 'mapX2'], o = i[yMapProp];
                    
                    e.strokeStyle = "rgba(79, 236, 255, 0.4)";
                    e.lineWidth = 1.5;
                    e.beginPath();
                    e.arc(t, o, 8 + Math.sin(Date.now() / 150) * 2, 0, 2 * Math.PI);
                    e.stroke();

                    const r = 7;
                    let skinImg = getSkinImg(i[skinProp]);
                    if (skinImg) {
                        e.save();
                        e.beginPath();
                        e.arc(t, o, r, 0, 2 * Math.PI);
                        e.closePath();
                        e.clip();
                        e.drawImage(skinImg, t - r, o - r, r * 2, r * 2);
                        e.restore();
                        e.lineWidth = 1.5;
                        e.strokeStyle = "#4fecff";
                        e.beginPath();
                        e.arc(t, o, r, 0, 2 * Math.PI);
                        e.stroke();
                    } else {
                        e.fillStyle = "#4fecff";
                        e.beginPath();
                        e.arc(t, o, 5, 0, 2 * Math.PI);
                        e.fill();
                    }
                    
                    let name = prefix + (i[nickProp] || "");
                    if (name.length > 0) {
                        e.font = "bold 13px Rajdhani, ubuntu, sans-serif";
                        e.textAlign = "center";
                        e.textBaseline = "bottom";
                        e.lineJoin = "round";
                        e.lineWidth = 2;
                        e.strokeStyle = "#000000";
                        e.strokeText(name, t, o - 10);
                        e.fillStyle = "#4fecff";
                        e.fillText(name, t, o - 10);
                    }
                };
                for (const i of se.teammates.values())
                    ...
                    i.isAlive) {
                        if (i.worldID === A.worldID || i.worldID === A.worldID2 || i.id === se.playerID || i.id === se.playerID2) {
                            ...
                            drawTeammate(i, 'x', 'mapY', 'nick', 'colorHex', 'skin');
                        }
                    };
                for (const i of ses.teammates.values()) {
                    if (i.isAlive && (2e3 < le.time - i.lastUpdateAt || 0 === i.mass)) i.isAlive = !1;
                    if (i.isAlive2 && (2e3 < le.time - i.lastUpdateAt || 0 === i.mass2)) i.isAlive2 = !1;
                    
                    if (i.isAlive) {
                        if (i.id === ses.saigoID) {
                            void 0;
                        } else {
                            drawTeammate(i, 'x', 'mapY', 'nick', 'colorHex', 'skin', oe.spyPrefix);
                        }
                    }
                    if (i.isAlive2) {
                        if (i.id === ses.saigoID) {
                            ...
                            drawTeammate(i, 'x2', 'mapY2', 'nick2', 'colorHex2', 'skin2', oe.spyPrefix);
                        }
                    }
				};
            }
    ...
    C = new ((()=>{
        ...
        return _createClass(e, [{
            key: "update",
            value() {
                200 > le.time - this.lastUpdateTime || (this.lastUpdateTime = le.time,
                this.generateList(),
    ...
    T = new ((()=>{
        ...
        return _createClass(e, [{
            ...
            value() {
                ...
                e += this.PIO + this.ping + this.paused + this.zoomLock,
            ...
            key: "ping",
            get() {
                const p1 = window._3rbPingValue;
                const p2 = window._3rbPingValue2;
                const has1 = window._3rbSlot1Sock && window._3rbSlot1Sock.readyState === 1;
                const has2 = window._3rbSlot2Sock && window._3rbSlot2Sock.readyState === 1;
                if (has1 && has2) {
                    const str1 = p1 !== undefined ? p1 + "ms" : "--";
                    const str2 = p2 !== undefined ? p2 + "ms" : "--";
                    return "Ping: " + str1 + "|" + str2 + "   ";
                } else if (has2) {
                    return "Ping: " + (p2 !== undefined ? p2 + "ms" : "--") + "   ";
                } else {
                    return "Ping: " + (p1 !== undefined ? p1 + "ms" : "--") + "   ";
                }
            }
        }, {
            key: "paused",
        ...
        return _createClass(e, [{
            ...
            value(e, t, o) {
                const i = 1 === o ? this.cellsTab1 : 2 === o ? this.cellsTab2 : 3 === o && this.cellsTab3
                  ...
                  , n = i.get(t)
                  , a = i.get(e);
                s.delete(t); // ALWAYS delete from myCellsTab to prevent ghost cells
                if (n) {
                    if (a) {
                        n.staticX = a.staticX;
                        n.staticY = a.staticY;
            ...
            value(e, t) {
                const o = 1 === t ? this.cellsTab1 : 2 === t ? this.cellsTab2 : 3 === t && this.cellsTab3
                  ...
                  , s = o.get(e);
                i.delete(e); // ALWAYS delete from myCellsTab to prevent ghost cells
                s && (o.delete(e),
    ...
    A = (()=>{
        ...
        return _createClass(e, null, [{
            ...
            key: "update",
            value() {
                if (0 < this.pieceCount) {
                    this.lastAliveTimeTab1 = Date.now();
                    this.playing();
                } else {
                    if (!this.lastAliveTimeTab1) this.lastAliveTimeTab1 = Date.now();
                    if (Date.now() - this.lastAliveTimeTab1 > 200) {
                        this.dead();
                    }
                }
                if (0 < this.pieceCount2) {
                    this.lastAliveTimeTab2 = Date.now();
                    this.playing2();
                } else {
                    if (!this.lastAliveTimeTab2) this.lastAliveTimeTab2 = Date.now();
                    if (Date.now() - this.lastAliveTimeTab2 > 200) {
                        this.dead2();
                    }
                }
                this.updateData()
                ...
                if (this.isAlive) {
                    ...
                    if (!this.movementPaused) {
                        const o = this.x - e
                          ...
                          , s = Math.sqrt(o * o + i * i);
                        this.speed += s,
                        this.x = (1 === this.controlledTab && this.isAliveTab1) ? e : (2 === this.controlledTab && this.isAliveTab2) ? e2 : (this.isAliveTab1 && this.isAliveTab2) ? (e + e2) / 2 : this.isAliveTab1 ? e : e2 || e,
                        this.x1 = e,
                        this.x2 = e2,
                        this.y = (1 === this.controlledTab && this.isAliveTab1) ? t : (2 === this.controlledTab && this.isAliveTab2) ? t2 : (this.isAliveTab1 && this.isAliveTab2) ? (t + t2) / 2 : this.isAliveTab1 ? t : t2 || t,
                        this.y1 = t,
                        this.y2 = t2
                    }
    ...
    O = (()=>{
        ...
        return _createClass(e, [{
            ...
            key: "mapX",
            get() {
                return (this.animX - G.left) * S.ratio
            }
        }, {
            key: "mapX2",
            get() {
                return (this.animX2 - G.left) * S.ratio
            }
        }, {
            key: "mapY",
            get() {
                return (this.animY - G.top) * S.ratio
            }
        }, {
            key: "mapY2",
            get() {
                return (this.animY2 - G.top) * S.ratio
            }
        }]),
        e
    })(),
    ...
    N = new ((()=>{
        ...
        return _createClass(e, [{
            ...
            value() {
				...
				// worldID → its skin URL so getCustomSkin() draws it. Full URLs (3rb CDN/imgur)
				// are used as-is; a bare imgur code falls back through code2url().
				if ("on" === k.skins3rb)
					for (const e of I.sortedCells)
						if (e.skin && !e.isVirus && !e.isEjected && !this.skinMap.has(e.worldID)) {
							var sk = e.skin, _skinUrl;
							var _skLower = sk.toLowerCase();
							if (_skLower === "removed" || _skLower === "none" || _skLower === "null") continue;
							if (sk.includes("://") || sk.startsWith("data:")) _skinUrl = sk;
							else if (sk.includes("imgur.com")) _skinUrl = "https://" + sk.replace(/^\/+/, "");
							else if (sk.startsWith("/")) _skinUrl = "https://3rb.io" + sk;
							else if (sk.startsWith("res/")) _skinUrl = "https://3rb.io/" + sk;
							else _skinUrl = "https://3rb.io/res/skins/" + sk + ".png";
							this.skinMap.set(e.worldID, _skinUrl);
						}
			}
        }, {
			key: "getKanjis",
            ...
            value(e, t) {
                ...
                for (var o = "on" === k.eatAnimation, i = e.readUInt16(); i--; ) {
                    ...
                    if (o) {
                        ...
                        // Even with eatAnimation off, we MUST remove eaten own-cells from
                        // myCellsTab so pieceCount reaches 0 and death/respawn triggers.
                        I.removeCell(s, t);
                    }
                }
                for (; !e.endOfBuffer; ) {
